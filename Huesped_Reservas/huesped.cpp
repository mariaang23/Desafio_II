...
void Huesped::asociarReservas(Reservas** listaReservas, int totalReservas){
    ...
    while (start < codigosReservas.length()) {
        ...
        if (end == string::npos) end = codigosReservas.length();

        string codReserva = codigosReservas.substr(start, end - start);

        // Limpieza de espacios al inicio y al final
        while (!codReserva.empty() && codReserva.front() == ' ')
            codReserva.erase(0, 1);
        while (!codReserva.empty() && codReserva.back() == ' ')
            codReserva.pop_back();

        if (!codReserva.empty()) {
            for (int i = 0; i < totalReservas; i++) {
                if (listaReservas[i]->getCodigoReserva() == codReserva) {
                    reservasHuesped[cantidadReservas++] = listaReservas[i];
                    break;
                }
            }
        }
...
void Huesped::mostrarReservasHuesped(){
    cout << "Reservas para huesped " << cedula << ":\n";
